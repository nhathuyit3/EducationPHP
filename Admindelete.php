...
<body>
	<div class="container">
		<?php
			...
			$sql = 'SELECT hocphan.ID, hocphan.TenHP, hocphan.SoTC, hocphan.giangvien FROM hocphan WHERE ID = "'.$_POST['id1'].'"';
			...
			while ($dl = $result->fetch_assoc()) {
				echo '<form action = "" method = "POST">
					...
					<div class = "form-group">
					<label>TenHP</label>
					<input type = "text" name = "GV" class="form-control" value="'.$dl['giangvien'].'"> 
					</div>
					<div class = "form-group">
					<label>SoTC</label>
					<input type = "text" name = "sotc1" class="form-control" value="'.$dl['SoTC'].'"> 
					</div>
					<button type="submit" name="submit" class="btn btn-outline-primary form-control" >Delete</button>
					</form> ';
		}
