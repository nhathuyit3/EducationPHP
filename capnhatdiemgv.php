...
<body>
    <div class="container">
        <?php
            ...
            $sql = 'SELECT loginstudent.ID,loginstudent.TenSV ,loginstudent.Diem FROM loginstudent WHERE MaSV = "'.$_POST['id'].'"';
             var_dump($sql);
            ...
            while ($dl = $result->fetch_assoc()) {
                echo '<form action = "" method = "POST">
                    ...
                    <input type = "text" name = "ten" class="form-control" value="'.$dl['TenSV'].'"> 
                    </div>
                    <div class = "form-group">
                    <label>Điểm</label>
        ...
        if (isset($_POST['submit'])) {
            ...
            $update = 'UPDATE svdkgv SET ID = "'.$id1.'", Diem = "'.$diem.'" WHERE ID = "'.$id1.'" ';
            $result = $conn->query($update);
        }
        ?>
        <td>
                     <input type="text" class="d-none" name="id" value='.$dl['MAGiangVien'].'>
                      <center><a href="SVDkGV.php"><button class="btn btn-outline-success">Trở lại</button></a></center>
                </td>
    </div>
     <!-- Optional JavaScript -->
